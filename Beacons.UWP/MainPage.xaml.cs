...
namespace Beacons.UWP
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        {
            grdPage.Background = new SolidColorBrush(Colors.White);
            lblDistance.Text = "Looking for the beacon ...";
            dt.Stop();
        }
        ...
        private async void OnAdvertisementReceived(BluetoothLEAdvertisementWatcher watcher, BluetoothLEAdvertisementReceivedEventArgs eventArgs)
        {
            ...
            Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () =>
            {
                if (dt.IsEnabled)
                    dt.Stop();
                lblDistance.Text = "Distance to Beacon " + Math.Round(beaconData.Distance, 1) + " m";
                grdPage.Background = new SolidColorBrush(ColorFromDistance(beaconData.Distance));
                dt.Start();
            });
        }
